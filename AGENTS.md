...
\- EXPLICIT TRIGGER: Requests for "templates", "boilerplates", or setup files are ALWAYS version-sensitive. You MUST run tools (`search\_web` or terminal commands) to fetch current latest versions and syntaxes before generating them. NEVER generate these from memory.
